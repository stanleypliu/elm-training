...
init session slug =
    ...
    , Cmd.batch
        [ Article.fetch maybeCred slug |> Http.toTask |> Task.attempt CompletedLoadArticle
        , Comment.list maybeCred slug |> Http.toTask |> Task.attempt CompletedLoadComments
        , Time.here |> Task.perform GotTimeZone
        , Loading.slowThreshold |> Task.perform (\() -> PassedSlowLoadThreshold)
        ]
    )
